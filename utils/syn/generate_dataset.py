...
if __name__ == '__main__':
	...
	source_dir = '/media/data/ywh/exploration_database_and_code/pristine_images'
	target_dir = '/media/data/ywh/CBDNet/data/train/synthetic_train_4744/'

	if not os.path.isdir(target_dir):
		os.makedirs(target_dir)

	fns = glob.glob(os.path.join(source_dir, '*.bmp'))
	...
	for fn in tqdm(fns):
		...
		for i in range(H_s):
			for j in range(W_s):
				...
				io.imsave(os.path.join(out_folder, 'SIGMA_SRGB_%d_%d.png' % (i, j)), sigma, check_contrast=False)
